...
- As you can see from the derivation, this magnetic moment μ is directly proportional to the angular momentum L. The proportionality constant is q/(2m), the charge-to-mass ratio of the electron.
...
The magnets are designed so that the field and its gradient are strongest in one direction, which we'll call the z-direction (vertical). This simplifies the force equation to Fz​=μz​(∂Bz​/∂z)

The vertical force Fz​ that deflects the atom is directly proportional to the z-component of its magnetic moment, μz​.

- Since we know the atom's mass m and we can measure the time t it spends in the magnet (from its velocity), we can use the simple equation of motion to relate the final deflection z(t) directly to a direct measurement of μz​, and therefore a direct measurement of Lz​, the z-component of the atom's angular momentum.
...
What they observed was shocking. The beam split cleanly into two distinct, separate spots. The atoms were deflected either 'up' or 'down' by a specific amount, and nothing in between. This result was a stunning confirmation of 'spatial quantization.' The angular momentum was clearly quantized.
...
- We define the inner product, written in 'bra-ket' notation as ⟨ϕ∣ψ⟩. This complex number represents the probability amplitude that a system prepared in state ∣ψ⟩ will be found in state ∣ϕ⟩ upon measurement
...
- Its action can be fully described by a matrix Oij​, which tells us how it transforms each basis vector. 
...
Operators that satisfy this are called Hermitian, and they are defined by the condition that they equal their own adjoint
...
If the commutator is zero, the operators 'commute,' and we call them compatible.
...
From the classical definition, we can write the component k of the angular momentum as ... where e_ijk is the levicivita tensor. 
...
However, the angular momentum squared, which is a scalar, does commute with any component.
This means we can find simultaneous eigenstates for L^2 and one of its components, by convention we pick L^z​.
...
These states have eigenvalues ℏ2s(s+1) and ℏms​. 

The number of ms​ states is 2s+1.
...
One final twist: to make the deflection magnitude match the experiment, the g-factor for spin gs​ had to be 2, not 1 like it was for orbital momentum. This 'anomalous' g-factor was another deep puzzle, which was later explained by Paul Dirac's relativistic quantum theory.
